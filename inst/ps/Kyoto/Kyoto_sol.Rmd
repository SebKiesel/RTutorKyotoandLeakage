...
library(yaml)
#library(restorepoint)
setwd("C:/Users/Hirschkopf/Documents/RTutorKyotoandLeakage/RTutorKyotoandLeakage/inst/ps/Kyoto")
ps.name = "Kyoto"; sol.file = paste0(ps.name,"_sol.Rmd")
libs = c("foreign","dplyr","lfe","stargazer","sandwich","XML","leaflet","regtools","AER","ggplot2","gridExtra","rworldmaps") # character vector of all packages you load in the problem set
name.rmd.chunks(sol.file) # set auto chunk names in this file
create.ps(sol.file=sol.file, ps.name=ps.name, user.name=NULL,libs=libs, stop.when.finished=FALSE, extra.code.file="extra.R" ,addons="quiz", use.memoise=TRUE, var.txt.file = "variables.txt", rps.has.sol=TRUE)
...
#< info "readRDS()"
The function `readRDS()` reads a file `data.rds` into a data frame.
If you set the working directory correctly and save the data in the same working path, it is enough to use the name of the data. The data will be saved under the name of the data, here `data`. Don't forget quotation marks around the dataset `data.rds`.
```{r "1",eval=FALSE}
readRDS("data.rds")
```

You can also use a complete data path if you like to store the data not in the working path.
```{r "1__2",eval=FALSE}
readRDS("C:/mypath/data.rds")
```

You can also store the data file under another name like `mydata`.
```{r "1__3",eval=FALSE}
mydata=readRDS("data.rds")
```

If you want to know more about the `readRDS()` command you can look at <a href="https://stat.ethz.ch/R-manual/R-devel/library/base/html/readRDS.html" target = "_blank"> stat.ethz.ch/R-manual/R-devel/library/base/html/readRDS.html</a>
...
#< hint
display("check the info box about the function readRDS")
#>
```

#< award "Dream Start"
  The Start was like a dream. 
  You can earn more awards in the following exercises, I hope you will have fun.
#>
...
head(dat)
#< hint
display("You need the dataset 'dat'")
#>
```
...
#>
#< hint
display("We already know the start of the dataset, to get the last year, try the function 'max()' with the column 'year'.")
#>
```

#< quiz "End of the Dataset"
question: The period of the dataset is from 1995 till...
answer: 2007
success: Great.
failure: No try again, the function 'max()' and the column 'year' could be helpful.
#>
<br>
...
#< info "filter()"
The function `filter()` contained in the `dplyr` package is used to generate a subset of a data frame. If you have a data set `dat` that contains a row `year` ranging from $1995$ until $2007$ and you want to generate a new data frame `dat_2000` that only contains information out of the year $2000$ you can use the following:
```{r "1__7",eval=FALSE}
library(dplyr)
dat_2000 = filter(dat, year == 2000)
```

If you want to know more about the function `filter()` you can take a look at <a href="https://cran.rstudio.com/web/packages/dplyr/vignettes/introduction.html" target = "_blank"> cran.rstudio.com/web/packages/dplyr/vignettes/introduction.html</a>.
...
#>
#< hint
display("the following code has everything you need for the question: min(filter(dat,kyotom!=0)$year)")
#>
```

#< quiz "Start of commitments"
question: At which year started the first commitment under Kyoto?
answer: 2001
success: Great, let Kyoto start!
failure: No, not right. If you need help, press the hint button on the code chunk before.
#>
<br>
...
#>
#< hint
display("the 'filter()' function could be helpful")
#>
```

#< quiz "variable sid"
question: For what stands the sector identifier 1000?
sc:
  - Trade between importer Estonia and exporter Australia in sector 2
  - Trade between importer Australia and exporter Canada in sector 7
  - Trade between importer Austria and exporter Switzerland in sector 4*
success: Great.
Failure: Try it again, if you need help, click on the hint button of the code chunk.
#>
<br>

#< quiz "sectors"
question: How many different sectors are in the dataset?
answer: 12
success: Great, sector 12 stands for non-specific industries.
failure: Try it again, if you need help, try the 'max()' function with the column 'sectors'.
#>
<br>
...
#>
#< hint
display("You need the function 'unique()' with the variable 'importer' ")
#>
```

#< quiz "intro2"
question: Which of the following countries take trade in our dataset?
mc:
  - Germany*
  - South Africa*
  - Tunisia
  ...
  - Qatar
success: Great.
failure: No, if you need help click the hint button of the code chunk before.
...
#< info "group_by()"
Some data operations are useful done in groups defined by variables in a dataset. With the function `group_by()` from the package `dplyr` you can convert an existing dataset into a grouped dataset. With this new dataset you can operate.

If you have a dataset `dat` and want to group this dataset by the variable `var` you can do it the following way:
```{r "1__11",eval=FALSE}
library(dplyr)
group_by(dat, var)
```

You can also save the grouped dataset under a variable `New_Data`:
```{r "1__12",eval=FALSE}
library(dplyr)
New_Data=group_by(dat, var)
```

You also can group the dataset with two variables, for example with the second variable `var2`:
```{r "1__13",eval=FALSE}
library(dplyr)
New_Data=group_by(dat, var, var2, add=FALSE)
```

With this new dataset you can do some operations like the functions `summarise()` or `filter()`. It is also possible to ungroup the dataset 'New_Data'. You can do it the following way with the function `ungroup()`:
```{r "1__14",eval=FALSE}
library(dplyr)
ungroup(New_Data)
```

If you want to know more about the function `group_by()` you can click on <a href="http://www.inside-r.org/node/230680" target="_blank">inside-r.org/node/230680</a>
#>
...
#< info "summarise()"
If you want to summarise multiple values to a single value the function `summarize()` is very helpful.

If you want to analyse the mean of a variable `var` in the dataset `dat` you can do it the following way:
```{r "1__15",eval=FALSE}
library(dplyr)
summarise(dat, mean(var))
```

You can also save the new variable under a specific name like `mean_var`.
```{r "1__16",eval=FALSE}
library(dplyr)
summarise(dat, mean_var=mean(var))
```

If you want additionally to analyse the standard deviation and the minimum of a second variable `var2`, also stored in the dataset 'dat', do it the following way:
```{r "1__17",eval=FALSE}
library(dplyr)
summarise(dat, mean(var), sd(var2), min(var2))
```

If you want to know more about the function `summarise()` you can look at <a href="http://www.inside-r.org/packages/cran/plyr/docs/summarise" target="_blank">inside-r.org/packages/cran/plyr/docs/summarise</a>
#>
...
#>
map2col(dat1,"Situation before Kyoto")
#< hint
display("map2col(...)")
#>
```
...
#>
#< hint
display("The following code delivers everything you need: length(unique(filter(dat, kyotom==1)$importer))")
#>
```

#< quiz "How many countries"
question: How many of the 40 countries are committed under Kyoto?
answer: 28
success: Great.
failure: Try it again, if you need help, the functions 'length()' and 'unique()' could be helpful or click on the hint button of the last code chunk.
#>
<br>
...
#< quiz "variables"
question: What is the meaning of the variable 'dk'?
sc:
  - the difference of kyotox and kyotom
  - the difference of kyotom and kyotox*
  - the detailed Kyoto targets
  - Donkey Kong
success: Great, -1 stands for the case, that only the exporter is committed, zero stands for both or no country is committed and 1 for only the importer is committed.
failure: Not right. Check the data description.
#>
<br>
...
dat2=dat%>%
  group_by(importer)%>%
  summarise(val=max(kyotom))
#< add_to_hint
display("here you need the maximum value of 'kyotom', and you should group the data by the importer country")
#>
```
...
#>
map2col(dat2, "Situation after Kyoto")
#< hint
display("map2col(...)")
#>
```
...
#...=filter(dat,...&...&...)
#>
gc=filter(dat, (importer=="Germany" | importer=="China") & (exporter == "Germany" | exporter=="China"))
#< hint
display("gc=filter(dat,(importer==\"...\"|importer==\"...\")&(exporter==\"...\"|exporter==\"...\"))")
#>
```
...
head(gc)
#< hint
display("The name of the dataset is 'gc'")
#>
```
...
#>
#< hint
display("the following codes give the solution: max(unique(filter(gc, importer==\"China\")$kyotom)) for China and with 'kyotox' for the exporter Germany, or you can click on the data button.")
#>
```

#< quiz "example status"
  question: Are our example countries Germany and China committed under Kyoto?
  sc:
    - No country is committed
    - Only China is committed
    - Only Germany is committed*
    - Both countries are committed
  success: Great. Only Germany is committed under Kyoto
  failure: Try again, if you need help click on the hint button of the last code chunk or click on the data button.
#>
<br>

#< quiz "Start of Germany"
question: When started the commitment of Germany?
answer: 2002
success: Great, the commitment of Germany started in the year 2002.
failure: No, try it again with the code 'min(filter(...)$year)'.
#>
<br>
...
#< info "select()"
The function `select()` contained in the `dplyr` package is used to select specific columns of a data frame. If you have a data set `dat` that contains columns `year`, `imports`, `country` and `exports` and you only want to access `year` and `exports`, you can do so with the following command:
```{r "1_1__5",eval=FALSE}
library(dplyr)
select(dat,year,exports)
```

You can also save the resulting dataset in a new variable called `dat_select`.
```{r "1_1__6",eval=FALSE}
library(dplyr)
dat_select=select(dat,year,exports)
```

If you want to know more about the function `select()` you can take a look at <a href="https://cran.rstudio.com/web/packages/dplyr/vignettes/introduction.html" target = "_blank"> cran.rstudio.com/web/packages/dplyr/vignettes/introduction.html</a>.
...
# ...(select(gc,...))
#>
head(select(gc, year, importer, exporter, imp, exp))
#< hint
display("head(select(gc,...))")
#>
```
...
#>
#< hint
display("With the following command you can get all information you need for the first two questions:
        summarise(filter(example, importer==\"China\"), mean(imp), mean(exp))")
#>
```


#< quiz "imports"
question: Which of these two countries have more imports from the other country in the complete period and over all sectors?
answer: Germany
success: Great.
failure: Try again. If you need help, click the hint button on the above code chunk.

#>
<br>

#< quiz "imports2" 
question: How much is the mean of these imports from China to Germany in one year over all sectors?
sc:
  - Over 170 billion US-Dollars*
  - Under 170 billion US-Dollars
  - Under 170 billion Euros
success: Yeah! Germany imported goods of more than 170 billion Us-Dollars.
failure: No, try again. If you need help, click on the hint button of the last code chunk.
#>
<br>

#< award "You should go to a quiz show"
With this energy you could win the million in a quiz show.
#>
...
#>
gc%>%
  filter(importer=="Germany")%>%
  group_by(kyotom)%>%
  summarise(mean(imp), mean(exp))
#< add_to_hint
display("First you need the importer Germany, then the variable 'kyotom' and then the means of the imports and the exports.")
#>
```
...
#< add_to_hint
display("You should group the data by the year and importer country and want a summary of the mean of the imports.")
#>
#< task
#enter your code here
#>
head(a1)
#< hint
display("show the head of the result")
#>
```
...
# ...(select(gc,...))
#>
head(select(gc, year, importer, exporter, BEIM, BEEX))
#< hint
display("head(select(gc, ...))")
#>
```
...
gc%>%
  filter(importer=="Germany")%>%
  group_by(kyotom)%>%
  summarise(mean(BEIM), mean(BEEX))
#< add_to_hint
display("You need the mean of the variables 'BEIM' and 'BEEX'. If you need more help, look at the Exercise before.")
#>
```
...
a1=gc%>%
  group_by(year, importer)%>%
  summarise(BEIM=mean(BEIM), BEEX=mean(BEEX))
#< add_to_hint
display("You need the means of the bilateral emissions embodied in imports and exports, respectively")
#>
#< task
# enter your code here
#>
head(a1)
#< hint
display("Show the head of the resulting dataset")
#>
```
...
#>
head(select(gc, year, importer, exporter, int_imp, imp, BEIM))
#< hint
display("don't forget the function 'head()'")
#>
```
...
a1=gc%>%
    group_by(year,importer)%>%
    summarise(int_imp=mean(int_imp))
#< add_to_hint
display("if you need help, look at the exercises before")
#>
#< task
# enter your code here
#>
head(a1)
#< hint
display("Show the first rows with the function 'head()'")
#>
```
...
#>
#< hint
display("you can group the data by 'statusx' and then filter the dataset to years until 2001 or for the second question to the years after.")
#>
```

#< quiz "content before"
question: How big are the bilateral emissions embodied in imports in the mean from committed exporter countries in relation to the mean of the carbon content of the imports from non-committed exporter countries before the start of the Kyoto commitments till the year 2000?
sc:
 - ca. twice as much
 - ca. equal*
 - ca. half as much
success: Great, the carbon content is circa equal from committed and non-committed exporters.
failure: No, try again. Look at the chunk above for a hint.
#>
<br>

#< quiz "carbon emission"
question: How big are the bilateral emissions embodied in imports in the mean from committed exporter countries in relation to the mean of the carbon content of the imports from non-committed exporter countries after the start of the Kyoto Protocol in the year 2001?
sc:
  - ca. twice as much
  - ca. equal
  - ca. half as much*
success: Great, after Kyoto the carbon content is now circa half as much.
failure: No, try again, if you need help look at the chunk above.
#>
<br>
...
#>
a1=dat%>%
  group_by(year,statusx)%>%
  summarise(int_imp=mean(int_imp))
#< add_to_hint
display("Group the dataset by the variables 'year' and the indicatorvariables 'statusm'.")
#>
```
...
#< award "Data Inspector"
  I can see a new Sherlock Holmes
#>
<br>
...
#< quiz "impact1"
question: Which influence do you think the variable 'dk' has in this naive model?
sc:
  - With increasing value of 'dk' will lower the imports
  - With increasing value of 'dk' will increase the imports
  - No significant influence*
success: Great.
failure: No, try again. The variable 'dk' describes the difference of the Kyoto status of the importer and the exporter, do you think that this variable alone is enough to predict the imports?
#>
<br>
...
#< info "lm()"
The `lm` function can be used to do some linear regressions. If you want to regress `y` with `x1` and `x2` all stored in the dataset `dat` you can use the following code:
```{r "2__2",eval=FALSE}
lm(y~x1+x2, data=dat)
```

You also can store the result in a variable. If you want to store the regression under `reg` you can do it the following way, all variables are again in the dataset `dat`:
```{r "2__3",eval=FALSE}
reg=lm(y~x1+x2, data=dat)
```

If you want to know more about the `lm()` function you can take a look at <a href="https://stat.ethz.ch/R-manual/R-devel/library/stats/html/lm.html" target = "_blank"> stat.ethz.ch/R-manual/R-devel/library/stats/html/lm.html</a>
#>
...
# reg1=lm(...~..., data=)
#>
reg1=lm(imp~dk+year, data=dat)
#< hint
display("reg1=lm(...~dk+year, data=...)")
#>
```
...
#< info "stargazer()"
If you want to get a well looking summary statistic of a regression `reg` you can use the `stargazer()` function from the package `stargazer`.
```{r "2__5",eval=FALSE}
library(stargazer)
stargazer(reg)
```

There is also an extra option for the HTML version, which we need in this problem set for the regression `reg`.
```{r "2__6",eval=FALSE}
library(stargazer)
stargazer(reg, type="html")
```

You can also get summary statistics of more than one regression at the same time to compare the regressions and you can label these summary statistics. For example you have three regressions `reg1`, `reg2` and `reg3` and you want to name it `regression results` you can do it the following way:
```{r "2__7",eval=FALSE}
library(stargazer)
stargazer(reg1, reg2, reg3, title="regression results", align=TRUE, type="html")
```

If you want to know more about the `stargazer()` function you can take a look at <a href="https://cran.r-project.org/web/packages/stargazer/vignettes/stargazer.pdf" target="_blank">cran.r-project.org/web/packages/stargazer/vignettes/stargazer.pdf</a>
#>
...
#< info "statistics"
The standard error is an estimate of how far the sample mean is likely to be from the population mean. If the population standard deviation is finite, then the standard error of the sample mean will tend to zero with increasing sample size.

$R^2$ is the coefficient of determination and indicates the proportion of the variance in the dependent variable that is predictable within the independent variables of the model. It is used to test statistic models which want to predict some future outcomes or to test a hypothesis, on basis of other related information. The given value is a measure of how well the observed outcomes are replicated by the model compared to the total variation of outcomes explained by the model. 

The adjusted $R^2$ is an approach to take account of the phenomenon, that $R^2$ automatically increase by taking extra explanatory variables into the model. The adjusted $R^2$ can be negative, and its value will always be smaller or equal to $R^2$. The adjusted value only increases if the $R^2$ increases more than one would expect to see by chance. The adjusted $R^2$ is defined as:

$$\bar{R}^2 = 1 - (1 - R^2) \cdot \frac{n - 1}{n - p} = R^2 - (1 - R^2) \cdot \frac{p - 1}{n - p}$$
...
An F-Test is a statistical test. The test statistic has an F-distribution under the null hypothesis. Often it will be used to compare two different models to identify the better one which best fits the population from which the data were sampled. To reject the null hypothesis, the F-value should be minimum over $0.1$.
#>
...
#>
head(select(dat, year, importer, exporter, gdpm, gdpx))
#< hint
display("head(select(...))")
#>
```
...
#< quiz "impact2"
question: Which impact on the imports do you think the GDP of the exporter and the importer have?
mc:
  - With increasing GDP of the exporter will lower the imports
  - With increasing GDP of the exporter will increase the imports*
  - With increasing GDP of the importer will lower the imports
  - With increasing GDP of the importer will increase the imports*
success: Great, a higher GDP normally increase the imports and also the exports of a country.
...
#>
reg2=lm(imp~dk+gdpm+gdpx+year, data=dat)
#< hint
display("You need the variables 'gdpm' and 'gdpx'.")
#>
```
...
#< quiz "variables2"
question: What is the meaning of the variable 'wto'?
sc:
  - It's an indicator variable for joint WTO membership*
  - It`s an indicator variable for WTO membership of the importer
  - It`s an indicator variable for WTO membership of the exporter
success: Great. There are other variables like this, the variable 'eu' stands for joint EU membership and the variable 'fta' for a free trade agreement.
failure: That's wrong, check the data description and try it again.
#>
<br>
...
# ...(select(dat, importer, exporter,...))
#>
head(select(dat, importer, exporter, fta, wto, eu))
#< hint
display("head(select(...))")
#>
```
...
#< quiz "impact3"
question: Which impact do you think a free trade agreement have on the quantity of imports? The FTA is a free trade agreement between its members.
sc:
  - A free trade agreement will lower the imports
  - A free trade agreement will increase the imports*
  - A free trade agreement has no significant influence
success: Great.
failure: Try again.
#>
<br>
...
#>
reg3=lm(imp~dk+gdpm+gdpx+fta+wto+eu+year, data=dat)
#< hint
display("You need the variables 'fta', 'wto', and 'eu'.")
#>
```
...
# enter your code here
#>
head(select(dat, year, importer, exporter, dist, comlang, contig))
#< hint
display("head(select(...))")
#>
```
...
#>
reg4=lm(imp~dk+gdpm+gdpx+fta+wto+eu+dist+contig+comlang+year, data=dat)
#< hint
display("look at the last exercise, but here also with the three variables 'dist', 'contig' and 'comlang'")
#>
```
...
#>
reg5=lm(imp~dk+gdpm+gdpx+fta+wto+eu+mrdis+mrcon+mrcom+mrwto+mrfta+mreu+dist+contig+comlang+year, data=dat)
#< hint
display("The same like in the exercise before, only with the additional variables for the multilateral resistance.")
#>
```
...
# datlog=readRDS(...)
#>
datlog=readRDS("logtrades.rds")
#< hint
display("The name of the dataset is 'logtrades.rds'")
#>
```
...
#>
reg6=lm(limp~dk+lgdpm+lgdpx+fta+wto+eu+mrdis+mrcon+mrcom+mrwto+mrfta+mreu+ldist+contig+comlang+year, data=datlog)
#< hint
display("It is similar to the last exercise, only with the logarithm of the variables: 'limp', 'lgdpm', 'lgdpx' and 'ldist' all variables in the new dataset 'datlog'.")
#>
```
...
The `felm()` function out of the package `lfe` is used for extracting linear group fixed effects. It basically works like the standard `lm()` function but offers some additional features. I will only explain the functionalities we use here.

If you want to regress `y` on `x1` and `x2`, all stored in the dataset `dat`, just like in the function `lm()`, you can use the following code:
```{r "2__24",eval=FALSE}
library(lfe)
felm(y~x1+x2, data=dat)
```

If you want to perform a regression with fixed effects on the variable `fixed_eff`, you can use the following formula:
...
To find out more about the function `felm()` you can look at here <a href="http://www.rdocumentation.org/packages/lfe/functions/felm" target = "_blank"> rdocumentation.org/packages/lfe/functions/felm</a>
#>
...
#>
felm1=felm(limp~dk+lgdpm+lgdpx+fta+wto+eu+mrdis+mrcon+mrcom+mrwto+mrfta+mreu+ldist+comlang+contig | sid + year, data=datlog)
#< hint
display("You need the function 'felm(), if you need help, click on the info box above.")
#>
```
...
#< info "felm()2"
With the function `felm()` from the package `lfe` you can also do some instrumental regressions. If you want to estimate a variable `x` with three variables `y1`, `y2` and `y3`. But `y1` is endogenous, and you want to estimate `y1` with `y2` and minimum one other variable like `z1` and `z2`, all stored in the dataset `dat` and as fixed effect you can use the variable `fixed_eff`.

```{r "2__28",eval=FALSE}
library(lfe)
felm(x~y2+y3 | fixed_eff |(y1~y2+z1+z2), data=dat)
```
#>
...
#>
felm2=felm(limp~lgdpm+lgdpx+fta+wto+eu+mrdis+mrcon+mrcom+mrwto+mrfta+mreu | sid + year | (dk ~ dICC+lgdpm+lgdpx+fta+wto+eu+mrdis+mrcon+mrcom+mrwto+mrfta+mreu+year), data=datlog)
#< hint
display("The basic structure is given, the new variable is 'dICC'.")
#>
```
...
#>
felm3=felm(limp~dk+fta+wto+eu | sid+year+importer+exporter, data=datlog)
#< hint
display("Now we have four fixed effects and you don't need some variables, for more help look at the regression formula")
#>
```
...
#< award "felm-King"
  You can handle the felm package now on your own, that's genius.
#>
...
#>
felm_imp=felm(limp~dk+fta+wto+eu | sid+year+importer+exporter, data=dat)
#< hint
display("felm_imp=felm(...~dk+fta+wto+eu | sid+year+importer+..., data=dat)")
#>
```
...
#dat1=filter(...)
#>
dat1=filter(dat, year>=1997 & year<=2000)
#< hint
display("dat1=filter(..., year>=1997 & ...")
#>
#< task
#dat2=filter(dat, ...)
#>
dat2=filter(dat, year>=2004 & year<=2007)
#< hint
display("dat2=filter(..., year>=2000 & ...")
#>
```
...
#< hint
display("Do exactly the same like in the exercise before, only for the second dataset dat2")
#>
#< task
# enter your code here
#>
...
#< add_to_hint
display("Look at the Exercise before")
#>
```
...
p1
#< hint
display("Just call the name of the plot, here p1")
#>
```
...
quantity1=dat1%>%
  group_by(importer)%>%
  summarise(val=mean(imp))
#< add_to_hint
display("Name the dataset 'quantity1', group by the importer country and get the means of the imports, not the logarithm of the imports.")
#>
```
...
#>
map4col(quantity1, "Quantity before Kyoto ")
#< hint
display("map4col(...)")
#>
```
...
quantity2=dat2%>%
  ...
  summarise(val=mean(imp))
#< add_to_hint
display("name it 'quantity2', don't use the logarithm of the imports")
#>
```
...
#>
map4col(quantity2, "Quantity after Kyoto")
#< hint
display("map4col(...)")
#>
```
...
#...=felm(...~... | sid+..., data=dat)
#>
felm_int=felm(lint~dk+fta+wto+eu | sid+year+importer+exporter, data=dat)
#< hint
display("felm_int=felm(...~dk+fta+wto+eu | sid+year+importer+exporter, data=dat)")
#>
```
...
p2=changes("lint",dat)
#< hint
display("If you need help, look at the exercises before and/or in the function note")
#>
#< task
# enter your code here
#>
...
intensity1=dat1%>%
  group_by(importer)%>%
  summarise(val=mean(int_imp))
#< add_to_hint
display("remember the variable for the intensity is 'int_imp'")
#>
```
...
#>
map4col(intensity1, "Intensity before Kyoto")
#< hint
display("map4col(...)")
#>
```
...
intensity2=dat2%>%
  ...
  summarise(val=mean(int_imp))
#< add_to_hint
display("the variable for intensity is 'int_imp', don't forget the percentage signs.")
#>
```
...
#>
map4col(intensity2, "Intensity after Kyoto")
#< hint
display("map4col(...)")
#>
```
...
#...=felm(...~... | sid+..., data=dat)
#>
felm_beim=felm(lbeim~dk+fta+wto+eu | sid+year+importer+exporter, data=dat)
#< hint
display("felm_beim=felm(~dk+fta+wto+eu | sid+year+importer+exporter, data=dat)")
#>
```
...
#enter your code here
#>
p3=changes("lbeim",dat)
#< hint
display("If you need help, look at the exercises before and/or on the note with the implementation of the function.")
#>
#< task
# enter your code here
#>
...
content1=dat1%>%
  group_by(importer)%>%
  summarise(val=mean(BEIM))
#< add_to_hint
display("The variable for carbon content is 'BEIM'")
#>
```
...
#>
map4col(content1, "Carbon content before Kyoto")
#< hint
display("map4col(...)")
#>
```
...
content2=dat2%>%
  ...
  summarise(val=mean(BEIM))
#< add_to_hint
display("The variable for the carbon content is 'BEIM')")
#>
```
...
#>
map4col(content2, "Carbon content after Kyoto")
#< hint
display("map4col(...)")
#>
```
...
#< quiz "Expectation"
question: What do you expect for the influence of `dk` to `lant`?
sc:
    - Smaller than zero*
    - Circa equal to zero
    - Greater than zero
success: yes, it should be smaller than zero.
failure: no, try again.
#>
<br>
...
#...=felm(...~... | sid+..., data=dat)
#>
felm_ant=felm(lant~dk | sid+year+importer+exporter, data=dat)
#< hint
display("felm_ant=felm(lant~dk | sid+year+importer+exporter, data=dat)")
#>
```
...
p4=changes("lant",dat)
#< hint
display("If you need help, look at the exercises before and/or in the note about the implementation of the function")
#>
#< task
# enter your code here
#>
...
antweiler1=dat1%>%
  group_by(importer)%>%
  summarise(val=mean(ant))
#< add_to_hint
display("The Antweiler variable is 'ant'")
#>
```
...
#>
map4col(antweiler1, "Antweiler variable before Kyoto")
#< hint
display("map4col(...)")
#>
```
...
#   summarise(val=...)
#>
dat2=filter(dat2, ant!="NA")
antweiler2=dat2%>%
  group_by(importer)%>%
  summarise(val=mean(ant))
#< add_to_hint
display("Don't forget to delete the rows with a not available value")
#>
```
...
#>
map4col(antweiler2, "Antweiler variable after Kyoto")
#< hint
display("map4col(...)")
#>
```
...
#< hint
display("grid.arrange(p1,...,nrow=2)")
#>
```

#< award "MasterPlotter"
  I see some pictures like from Picasso. It's amazing.
#>
...
According to Cobb-Douglas, this is the most standard form of producing one good:

$$ Y = A \cdot L^\beta \cdot K^\alpha$$

  - $Y$ is the total output of the production.
  - $A$ is the total factor productivity.
  - $L$ is the labour input.
  - $K$ is the invested capital.
  - $\alpha$ and $\beta$ are the elasticities for capital and labour, respectively. These two constant values are determined by available technology.

Some examples can explain it better:
  
  - $\alpha = 0.45$, an increase in capital of $1\%$, would lead to approximately an increase in output of $0.45\%$.
 
  - $\alpha + \beta = 1$ has constant returns to scale. Doubling of the capital $K$ and the labour $L$ would also doubling the output $Y$. 
  
  - $\alpha + \beta < 1$ has decreasing returns to scale. 
  
  - $\alpha + \beta > 1$ has increasing returns to scale. In this case you could always increase the output by increasing labour and/or capital.

Assuming perfect competition, then $\alpha + \beta = 1$ can be shown to be capital's and labour's shares of output. Cobb and Douglas showed that labour and capital shares of the total output of production were constant over time in developed countries by statistical evidence. 

If you want to know more about Cobb-Douglas you can take a look at Biddle (2012).
#>
...
#< info "Elasticity of Substitution"
The elasticity of substitution means the elasticity of the ratio of two inputs to a utility or productivity function with respect to the ratio of their utilities and products, respectively. In perfect competition it measures the percentage change in the ratio of the used two inputs in response to a percentage change in their prices. And it measures, how easy it is to substitute one input or one good for the other one. 
...
#< info "iceberg trade cost factor"
The iceberg trade cost factor is normally a cost of transporting a good that uses up a fraction of the good itself, rather than other resources. Through the transport of a good, the icberg melts and the good isn't such valuable like before the transportation, the iceberg melts. Far from realistic, but a good way of modelling transportation costs. Here the costs are added to the price for the importer, which has to pay for the transportation.

If you want to know more about the iceberg trade cost factor you can take a look at Irarrazabal et al. (2015).
#>
...
#< quiz "Formula 1"
question: Which price index is dual to the manufacturing goods?
sc: 
    - The mill (ex-factory) price of a differentiated good
    - The sectoral price index
    - The general price index*
success: Yeah it is very good.
failure: No, try it again.
#>
...
#< quiz "formula 2"
question: What is part of the cost function of a firm?
mc:
    - Labour costs*
    - Regulation to control emission intensity*
    - Output level of the firm
success: Yeah, very good.
failure: Not correct, one option isn't part of the cost function, you need this option only for the overall costs.
#>
...
mc:
    - The demand structure on varieties is a Cobb-Douglas composition.
    - We need a multiplier for international trade in the formula of the optimal size of the firm*
    - The supply capacity of the importer is part of the formula for the quantity of the importer country.
success: Great, you are right.
failure: No, that's not correct
#>
...
#< quiz "formula 4"
question: For what stands MRIO?
sc:
    - Method Regional Input Output
    - Multi Region Input Output*
    - Michael Roberts and Ian O'Neill
success: Great, it is correct.
failure: not correct, try it again.
#>
...
mc:
    - The stronger the elasticities are, the stronger is the reaction on stricter climate policy*
    - Technique effects are only driven by domestic carbon taxes.
    - Without transition countries the cost function only depends on domestic carbon taxes.*
success: very good.
...
#>
felm_imp=felm(limp~kyotom+kyotox+lgdpm+lgdpx+fta+wto+eu+mrdis+mrcon+mrcom+mrwto+mrfta+mreu | sid+year, data=dat)
#< hint
display("Do not use the variable 'dk' here, you should use the variables 'kyotom' and 'kyotox'")
#>
```
...
#...=felm(...~...+mrdis+mrcon+mrcom+mrwto+mrfta+mreu | sid+year, data=dat)
#>
felm_int=felm(lint~kyotom+kyotox+lgdpm+lgdpx+fta+wto+eu+mrdis+mrcon+mrcom+mrwto+mrfta+mreu | sid+year, data=dat)
#< hint
display("estimate with the variables 'kyotom' and 'kyotox' and not with the variable 'dk'")
#>
```
...
# enter your code here
#>
kappaQm=coef(felm_imp)[1]
#< hint
display("the function coef() is helpful to get the coefficients of a regressions, here you need: coef(felm_imp)[1], and don't forget that the technique effect for the importer was not significant, so it is equal to zero.")
#>
#< task
# enter your code here
#>
kappaQm
#< hint
display("Show the value of kappaQm.")
#>
```
...
# enter your code here
#>
kappaQx=coef(felm_imp)[2]-coef(felm_int)[2]
#< hint
display("...=coef(...)[2]-coef(...)[2]")
#>
#< task
# enter your code here
#>
kappaQx
#< hint
display("Show the value of 'kappaQx'")
#>
```
...
#< award "Formula-King"
  You solved this hard exercise, greetings.
#>
...
#>
dat_USA=filter(dat, dat$expin==1 | dat$impin==1 | (dat$nonein==1 & (dat$importer=="USA" | dat$importer=="Australia" | dat$importer=="Israel" | dat$importer=="South Korea" | dat$exporter=="USA" | dat$exporter=="Australia" | dat$exporter=="Israel" | dat$exporter=="South Korea")))
#< hint
display("First you need the indicator variables 'expin' and 'impin')")
#>
#< task
# length(...)
#>
length(dat_USA$dk)
#< hint
display("You need the function length()")
#>
```
...
#...=felm(...~... | sid+..., ...)
#>
felm_imp=felm(limp~dk+fta+wto | sid+year+importer+exporter, data=dat_USA)
#< hint
display("felm_imp=felm(limp~dk+fta+wto | sid+year+importer+exporter, data=dat_USA)")
#>
```
...
#< quiz "Estimation"
question: Which value has the adjusted R-squared?
sc:
  - 0.9115
  - 0.9032*
  - 0.9925
success: Great.
failure: No, try again, if you need help, look at the summary statistic, again.
#>
<br>
...
#...=felm(...~... | sid+..., ...)
#>
felm_int=felm(lint~dk+fta+wto | sid+year+importer+exporter, data=dat_USA)
#< hint
display("felm_int=felm(...~dk+fta+wto | sid+year+importer+exporter, data=...)")
#>
```
...
#...=felm(...~... | sid+..., ...)
#>
felm_beim=felm(lbeim~dk+fta+wto | sid+year+importer+exporter, data=dat_USA)
#< hint
display("felm_beim=felm(...~dk+fta+wto | sid+year+importer+exporter, data=...)")
#>
```
...
# enter your code here
#>
tail(select(dat, importer, exporter, dk_USA, dk, nonein))
#< hint
display("tail(select(dat, importer, exporter, dk_USA, dk, nonein))")
#>
```
...
#...=felm(...~... | sid+..., data=dat)
#>
felm_imp=felm(limp~dk_USA+fta+wto+eu | sid+year+importer+exporter, data=dat)
#< hint
display("felm_imp=felm(limp~...+fta+wto+eu | sid+year+importer+exporter, data=...)")
#>
```
...
#...=felm(...~... | sid+..., data=dat)
#>
felm_int=felm(lint~dk_USA+fta+wto+eu | sid+year+importer+exporter, data=dat)
#< hint
display("felm_int=felm(lint~...+fta+wto+eu | sid+year+importer+..., data=dat)")
#>
```
...
#...=felm(...~... | sid+..., data=dat)
#>
felm_beim=felm(lbeim~dk_USA+fta+wto+eu | sid+year+importer+exporter, data=dat)
#< hint
display("felm_beim=felm(...~...+fta+wto+eu | sid+year+importer+..., data=dat)")
#>
```
...
# head(select(dat,...))
#>
head(select(dat, lint, lint2))
#< hint
display("head(select(dat, lint, lint2))")
#>
```
...
#...=felm(...~... | sid+..., data=dat)
#>
felm_int2=felm(lint2~dk+fta+wto+eu | sid+year+importer+exporter, data=dat)
#< hint
display("felm_int2=felm(lint2~dk+fta+wto+eu | sid+year+importer+exporter, data=dat)")
#>
```
...
#>
felm1=felm(ltech~kyotom+kyotox+lgdpm+lgdpx+fta+wto+eu+mrdis+mrcon+mrcom+mrwto+mrfta+mreu | sid+year, data=dat)
#< hint
display("You need the variables from the first option of the third exercise.")
#>
```
...
#>
felm2=felm(ltechmrio~kyotom+kyotox+lgdpm+lgdpx+fta+wto+eu+mrdis+mrcon+mrcom+mrwto+mrfta+mreu | sid+year, data=dat)
#< hint
display("we generated the needed variable in the second last exercise")
#>
```
...
#< award "Mr. Alternative"
  You are so good, you are not normal, you are more than an alternative
#>
...
#>
#< hint
display("delete the commentary sign and change the cat into a number between 1 and 12")
#>
```


#< award "You are unbelievable"
  You are fantastic. Every single sector is now estimated. I take my hat off.
#>
